...
public class Soap implements SoapInterface {
	...
	public OMElement soapCall(OMElement body, Protocol protocol,
			String endpoint, boolean mtom, boolean addressing, boolean soap12,
			String action, String expected_return_action) throws XdsException,
			AxisFault {
	...
	public void soapSend(OMElement body, Protocol protocol, String endpoint,
			String action) throws XdsException, AxisFault {
		...
		try {
			serviceClient.getOptions().setFrom(
					new EndpointReference(InetAddress.getLocalHost()
							.getHostAddress()));
		...
		if (addressing) {
			...
			serviceClient.disengageModule("addressing"); // this does not work
															// in Axis2 yet
		}
	...
	public OMElement soapCall(OMElement body, Protocol protocol,
			String endpoint, String action) throws XdsException, AxisFault {
		...
		serviceClient.getOptions().setProperty(
				HTTPConstants.CUSTOM_PROTOCOL_HANDLER, protocol);
		...
		Object in = serviceClient.getServiceContext().getLastOperationContext()
				.getMessageContexts().get("In");
		if (!(in instanceof MessageContext))
			throw new XdsInternalException("Soap: In MessageContext of type "
					+ in.getClass().getName() + " instead of MessageContext");
	...
	private ConfigurationContext getContext() throws AxisFault {
		String axis2Repository = PropertyFacade.getString("axis2.repo.dir", "./conf/axis2repository");
		String axis2Config = PropertyFacade.getString("axis2.xml", "./conf/axis2repository/axis2.xml");
		if (logger.isDebugEnabled()) {
			logger.debug("Using axis2.repo.dir of " + axis2Repository + " and axis configuration file of " + axis2Config);
		}
		ConfigurationContext configctx = ConfigurationContextFactory
				.createConfigurationContextFromFileSystem(axis2Repository, axis2Config);
		return configctx;
	}
	...
	void verify_returned_action(String expected_return_action,
			String alternate_return_action) throws XdsException {
