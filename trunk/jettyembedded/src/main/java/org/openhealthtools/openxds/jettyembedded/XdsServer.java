...
package org.openhealthtools.openxds.jettyembedded;
...
public class XdsServer {
    ...
    private void startContainer() throws Exception {
        ...
        Context rootContext = new Context(server, "/" + context, Context.ALL);
        rootContext.setResourceBase(root);
        ServletHolder holder = new ServletHolder(new AxisServlet());
        rootContext.addServlet(holder, "/services/*");
        holder.setInitParameter("axis2.repository.path", root + "/WEB-INF/");
        holder.setInitParameter("axis2.xml.path", root + "/WEB-INF/conf/axis2.xml");

        ServletHolder holder1 = new ServletHolder(new ConfigServlet());
        rootContext.addServlet(holder1, "/ConfigServlet");

        org.apache.jasper.servlet.JspServlet jsp = new org.apache.jasper.servlet.JspServlet();
        rootContext.addServlet(new ServletHolder(jsp), "*.jsp");
