...
public class XdsFactory {
	...
	private static XdsFactory SINGLETON;
	...
	private XdsFactory() {
		try {
			log.debug("Loading context configuration from: " + Arrays.toString(getConfigLocations()));
	...
	public synchronized static XdsFactory getInstance() {
		if (SINGLETON != null) {
			return SINGLETON;
		}
		configLocations = new String[] { "classpath*:/*applicationContext-openxds.xml" };
		SINGLETON = new XdsFactory();
		return SINGLETON;
	}

	public synchronized static XdsFactory getInstance(String home) {
		if (SINGLETON != null) {
			return SINGLETON;
		}
		configLocations = new String[] { "file:" + home + "/applicationContext-openxds.xml" };
		SINGLETON = new XdsFactory();
		return SINGLETON;
	}
	...
	/**
	 * The factory method to get {@link XdsRepositoryItem}. RepositoryItem is
	 * non-singleton bean.
	 * 
	...
	public static XdsRepository getRepositoryActor() {
		...
		for (XdsRepository actor : actors) {
			...
			if (isSecure && isSecureActor || !isSecure && !isSecureActor) {
				return actor;
			}
		}
		return ret;
	 ...
	 * 
	 * @param secure
	 *            whether the registry actor needs to be secure
	 * @return
	 */
