...
public class SubmitObjectsRequest extends XdsCommon {
	...
	short xds_version;
	protected final static Log logger = LogFactory.getLog(SubmitObjectsRequest.class);
	...
	public OMElement submitObjectsRequest(OMElement sor) {
		...
		try {
			...
			response.add_error("XDSRegistryDeprecatedDocumentError",
					"XDS Deprecated Document Error:\n " + e.getMessage(), RegistryUtility.exception_trace(e),
					log_message);
...
//			Publisher.getInstance().publish(sor, actor);
	...
	void submitObjectsRequestInternal(OMElement sor)
		...
		// this will set time on empty folders too. This is not required nor
		// forbidden by XDS
		...
		// If this submission includes a DocumentEntry replace and the original
		// DocumentEntry is in a folder
		// then the replacement document must be put into the folder as well.
		// This must happen here
		// so the following logic to update folder lastUpdateTime can be
		// triggered.

		addRPLCToFolder(m);

		// if this submission adds a document to a folder then update that
		// folder's lastUpdateTime Slot
		updateFolderTimes(m);
		...
		// add to the list of things to deprecate, any XFRM or APND documents
		// hanging off documents
		...
		// response.add_error("XDSRegistryError", e.getMessage(),
		// RegistryUtility.exception_details(e), log_message);
		// return;
		// }
		// catch (ParserConfigurationException e) {
		// response.add_error("XDSRegistryError", e.getMessage(),
		// exception_details(e), log_message);
		// return;
		// }
	...
	// this applies only to folders in the registry. folders in the submission
	// are handled separately
	protected void updateFolderTimes(Metadata m) throws MetadataException, LoggerException, XdsException,
			XdsInternalException, XMLParserException, MetadataValidationException {
		if (log_message != null)
		...
		for (OMElement assoc : m.getAssociations()) {
			...
			if (m.getFolderIds().contains(sourceId))
				continue; // sourceObject is folder in submission - handled
							// elsewhere
			...
			if (fm.getFolders().size() == 0)
				throw new XdsException("Adding to folder, sourceObject, " + sourceId
						+ ", is not a folder in submission or in Registry");
			// Set XDSFolder.lastUpdateTime
			String timestamp = new Hl7Date().now();
			...
			m.addMetadata(fm); // add to metadata collection, will get submitted
								// back along with new metadata

		}
	}

	private Metadata fetchFolderbyId(String sourceId) throws XMLParserException, LoggerException, XdsInternalException,
...
//	private boolean folderInSubmission(String sourceId) throws LoggerException, XdsException, XdsInternalException {
//		return new Structure(new Metadata(), false).isFolder(sourceId);
//	}

	protected void addRPLCToFolder(Metadata m) throws MetadataException, LoggerException, XdsException {
		...
		for (String replacementDocumentId : rplcToOrigIds.keySet()) {
			...
			// for each original document, find the collection of folders it
			// belongs to
			...
			// for each folder, add an association placing replacement in that
			// folder
			for (String fid : folderIds) {
				OMElement assoc = m.add_association(m.mkAssociation("HasMember", fid, replacementDocumentId));
				m.add_association(m.mkAssociation("HasMember", m.getSubmissionSetId(),
						assoc.getAttributeValue(MetadataSupport.id_qname)));
			}
		}
	}
	
	protected void addUpdatedDocToFolder(Metadata m, List<String> docIds)
			throws MetadataException, LoggerException, XdsException {

		String lid = m.getExtrinsicObjectLid();
		String id = m.getExtrinsicObjectId();
		if (log_message != null) {
			log_message.addOtherParam("Finding folders with old documents in it using lid: ", 
					lid);
		}

		Metadata me;
		for (String originalDocumentId : docIds) {

			// for each original document, find the collection of folders it
			// belongs to
			me = new SQFactory(this).findFoldersForDocumentByUuid(originalDocumentId, false /* LeafClass */);
			List<String> folderIds = me.getObjectIds(me.getObjectRefs());
			if (log_message != null)
				log_message.addOtherParam("RPLC containing folders=", folderIds.toString());
			// for each folder, add an association placing replacement in that
			// folder
			for (String fid : folderIds) {
				OMElement assoc = m.add_association(m.mkAssociation("HasMember", fid, id));
				m.add_association(m.mkAssociation("HasMember", m.getSubmissionSetId(),
						assoc.getAttributeValue(MetadataSupport.id_qname)));
			}
		}
	...
	protected void logIds(Metadata m) throws LoggerException, MetadataException {
		if (log_message != null)
	...
	protected void validateSourceId(Metadata m) throws MetadataException, MetadataValidationException {
		String sourceId = m.getExternalIdentifierValue(m.id(m.getSubmissionSet()),
		...
		if (!sourceIdValid)
			throw new MetadataValidationException(
					"sourceId " + sourceId + " is not acceptable to this Document Registry."
							+ " It must have one of these prefixes: " + sourceIds.toString());
	}

	protected void validate_patient_id(String patient_id) throws SQLException, XdsException, XdsInternalException {
		if (PropertyFacade.getBoolean("validate.patient.id")) {
			try {
				...
				if (!known_patient_id) {
					...
					throw new XdsUnknownPatientIdException("PatientId " + patient_id + " (Assigning Authority " + aa
	...
	/**
	 * Converts a patientId from CX format to an {@link PatientIdentifier}
	 * object.
	 * 
	 * @param patientId
	 *            the patient id, exp.
	 *            12321^^^&1.3.6.1.4.1.21367.2009.1.2.300&ISO
	...
	// private void validate_assigning_authority(String patient_id, Validator
	// val)
	// throws XdsException {
	...
	//// String config_assigning_authority =
	// Properties.loader().getString("assigning_authority");
	...
	protected boolean submit_to_backend_registry(String sor_string) throws XdsInternalException {
		boolean status = true;
	 ...
	 */
	protected void auditLog(String patientId, String submissionSetUid, AuditCodeMappings.AuditTypeCodes typeCode) {
		if (auditLog == null)
			return;
